...
| Duplicate UUID in vault | Create two notes with the same `exf` frontmatter value. | `Verify` reports duplicate-vault `Error`; mutating commands abort. |
| Duplicate UUID in external root | Create two bound folders with `.exf` files containing the same UUID. | `Verify` reports duplicate-external `Error`; mutating commands abort. |
| Malformed `.exf` | Add BOM, extra content, extra lines, or a non-canonical UUID to a marker. | Marker is classified as malformed `Error`; mutation preflight aborts. |

...
This procedure turns `docs/dev/plans/mvp.md` into an implementation sequence. Use it before coding any MVP item and before starting Phase 1 reconcile work.
...
Do not start Phase 1 reconcile while any Phase 0 done criterion is unresolved, unless the work is an isolated spike that will be deleted or reworked before merge.

## Phase 1 Reconcile Entry Gate

Before implementing reconcile, create a reconcile work note or pull request section that defines:
...
Reconcile is filesystem-moving work. Treat it as higher risk than Phase 0 folder creation.

## Reconcile Implementation Order

Implement reconcile in this order:
